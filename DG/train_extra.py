...
def main():
    ...
    parser.add_argument('--extra_dir_index', type=int, nargs='+', default=[0])
    ...
    args.unique_name = f"{timestamp}_{random.random()*100:.0f}_{args.name}"
    ...
    for test_env in args.test_envs:
        # change the dataset to be appended.
        args.extra_dir_index = test_env
        # args.extra_dir_index = 0
        res, records = train(
            test_env,
            args=args,
            hparams=hparams,
    ...
    table = PrettyTable(["Selection"] + [dataset.environments[i[0]] for i in args.test_envs] + ["Avg."])
