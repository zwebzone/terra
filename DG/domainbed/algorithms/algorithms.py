...
def parse_loss_file(filepath):
    ...
    with open(filepath, 'r') as file:
        for line in file:
            ...
            t = tuple(map(float, key.replace('t=(', '').replace(')', '').split(', ')))
            loss_dict[t] = float(val)
    return loss_dict


# def extract_t_and_weights(flat_paths, loss_dict):
#     max_loss = max(loss_dict.values())
#     min_loss = min(loss_dict.values())
#     weights = []

#     for path in flat_paths:
#         filename = path.split('/')[-1]
#         if 'time_t_' in filename:
#             # Extract t values
#             t_str = filename.split('time_t_')[-1].split('_seed')[0]
#             t = tuple(map(float, t_str.split('_')))
#             # Get the corresponding loss and convert to weight
#             loss = loss_dict.get(t, min_loss)  # Use min_loss if not found
#             weight = (loss - min_loss) / (max_loss - min_loss)
#         else:
#             weight = 1.0  # Default weight for images without t
#         weights.append(weight)

#     return torch.tensor(weights).cuda()  # Ensure this tensor is on the GPU
...
def extract_t_and_weights(flat_paths, loss_dict):
    ...
    for path in flat_paths:
        ...
        if 'time_t_' in filename:
            ...
            # Get the corresponding loss and convert to weight
            loss = loss_dict.get(t, min_loss)  # Use min_loss if not found
            # Normalize weight to range 0.1 to 1
            # weight = 0.9 * ((loss - min_loss) / (max_loss - min_loss)) + 0.1
            weight = ((loss - min_loss) / (max_loss - min_loss)) + 1.0
        else:
            weight = 1.0  # Default weight for images without t
        weights.append(weight)
...
def flatten_paths(nested_paths):
    # Flatten the nested list of paths
    flat_paths = [path for sublist in nested_paths for path in sublist]
    return flat_paths
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.extra_weight = self.hparams["extra_weight"]
    ...
    def update(self, x, y, **kwargs):
        ...
        extra_size = int(batch_size / self.num_domains)
        weight = torch.cat([torch.ones(batch_size - extra_size), torch.ones(extra_size) * self.extra_weight]).cuda()
    ...
    # def update(self, x, y, path, **kwargs):
    #     all_x = torch.cat(x)
    #     all_y = torch.cat(y)
    #     flat_paths = flatten_paths(path)
    #     # print(flat_paths)
    #     loss_dict = parse_loss_file('/data/home/zhangyl/code/UDA/Transfer-Learning-Library-master/examples/domain_generalization/image_classification/sam_losses_Ar_sincos_dim2.txt')
    #     weights = extract_t_and_weights(flat_paths, loss_dict)
    #     # print(weights)
    #     # Calculate weighted loss
    #     loss = F.cross_entropy(self.predict(all_x), all_y, reduction='none')
    #     weighted_loss = loss * weights  # Apply weights directly
...
class SAGM_DG(Algorithm):
    ...
    def update(self, x, y, **kwargs):
        ...
        def loss_fn(predictions, targets, weight):
            # return F.cross_entropy(predictions, targets)
            loss1 = F.cross_entropy(predictions, targets, reduction='none')
            loss1 = torch.mul(loss1, weight)
            loss1 = torch.mean(loss1)
            return loss1

        self.SAGM_optimizer.set_closure(loss_fn, all_x, all_y, weight)
...
class SAGM_DG_MixUP(SAGM_DG):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams)
        self.mixup_alpha = self.hparams["mixup_alpha"]
    ...
    def update(self, x, y, **kwargs):
        ...
        self.SAGM_optimizer.set_closure_mixup(loss_fn, x, y, self.mixup_alpha)
        predictions, loss = self.SAGM_optimizer.step()
        self.lr_scheduler.step()
        self.SAGM_optimizer.update_rho_t()
