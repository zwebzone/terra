...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    for step in range(n_steps):
        ...
        # }
        batches = {
    key: [tensor.to(device) if isinstance(tensor, torch.Tensor) else tensor for tensor in tensorlist]
    for key, tensorlist in batches.items()
        ...
        if step % checkpoint_freq == 0:
            ...
            # change the train_out to exclude the extra_domain.
            # if args.extra_dir!='None':
            #     extra_name = 'env'+str(dataset.environments.index("Extra"))+'_out'
            #     results['train_out'] = (results['train_out'] - results[extra_name]/(len(dataset)-1))*(len(dataset)-1)/(len(dataset)-2)
            # print
