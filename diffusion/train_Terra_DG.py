...
class DreamBoothDomainDataset():
    def __init__(
        ...
        # self.instance_data_root1 = "officehome/Art"
        # self.instance_data_root2 = "officehome/Clipart"
        # self.instance_data_root3 = "officehome/Product"
        # self.instance_data_root3 = "officehome/Real_World"
        ...
        # for stylename in training_styles:
        self.instance_images.extend([path for path in doublePath(self.instance_data_root1)])        
        self.custom_instance_prompts.extend([
            "A %s"% path.parent.name.replace("_", " ").lower()
            for path in doublePath(self.instance_data_root1)
        ])
        self.styinds.extend([0 for path in doublePath(self.instance_data_root1)])

        self.instance_images.extend([path for path in doublePath(self.instance_data_root2)])        
        self.custom_instance_prompts.extend([
            "A %s"% path.parent.name.replace("_", " ").lower()
            for path in doublePath(self.instance_data_root2)
        ])
        ...
        self.custom_instance_prompts.extend([
            "A %s"% path.parent.name.replace("_", " ").lower()
            for path in doublePath(self.instance_data_root3)
        ])
...
def main(args):
    ...
    t_pred_cnn = CustomResNet(num_classes=2)#预测一个一维的t

    t_pred_cnn.load_state_dict(torch.load("VLCS/t_pred_cnn_S.pt"))
    print("load finished!!!")
    ...
    t_pred_cnn.to(accelerator.device, dtype=torch.float32)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                t1 = t_pred_cnn(F.interpolate(pixel_values, size=(224, 224), mode='bilinear', align_corners=False))
...
# validation
                time_t = torch.tensor([0,0], device=accelerator.device, dtype=weight_dtype)
