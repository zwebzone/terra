...
class UNetMidBlock2D(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor, t1: torch.FloatTensor, temb: Optional[torch.FloatTensor] = None) -> torch.FloatTensor:
        hidden_states = self.resnets[0](hidden_states, t1, temb)
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if attn is not None:
                hidden_states = attn(hidden_states, t1=t1, temb=temb)
            hidden_states = resnet(hidden_states, t1, temb)
...
class UNetMidBlock2DCrossAttn(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.resnets[0](hidden_states, t1, temb, scale=lora_scale)
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
...
class DownBlock2D(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.FloatTensor, t1: torch.FloatTensor,temb: Optional[torch.FloatTensor] = None, scale: float = 1.0
    ) -> Tuple[torch.FloatTensor, Tuple[torch.FloatTensor, ...]]:
        output_states = ()
        ...
        for resnet in self.resnets:
            if self.training and self.gradient_checkpointing:
                ...
                if is_torch_version(">=", "1.11.0"):
                    hidden_states = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(resnet), hidden_states, t1, temb, use_reentrant=False
                    )
                else:
                    hidden_states = torch.utils.checkpoint.checkpoint(
...
class DownEncoderBlock2D(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor, t1: torch.FloatTensor, scale: float = 1.0) -> torch.FloatTensor:
        for resnet in self.resnets:
            hidden_states = resnet(hidden_states, t1, temb=None, scale=scale)
