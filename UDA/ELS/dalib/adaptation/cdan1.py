...
class ConditionalDomainAdversarialLoss(nn.Module):
    ...
    # def forward(self, g_s: torch.Tensor, f_s: torch.Tensor, g_t: torch.Tensor, f_t: torch.Tensor, weights_s) -> torch.Tensor:
    ...
    def forward(self, g_s: torch.Tensor, f_s: torch.Tensor, g_t: torch.Tensor, f_t: torch.Tensor, weights_s: torch.Tensor) -> torch.Tensor:
        f = torch.cat((f_s, f_t), dim=0)
        g = torch.cat((g_s, g_t), dim=0)
        g = F.softmax(g, dim=1).detach()
        h = self.grl(self.map(f, g))
        ...
        weights_t = torch.ones(g_t.size(0), 1).to(g_s.device)
        
        combined_weights = torch.cat((weights_s.view(-1, 1), weights_t), dim=0)
        ...
        weight = 1.0 + torch.exp(-entropy(g))
        batch_size = f.size(0)
        weight = weight * combined_weights.squeeze() 
        weight = weight / torch.sum(weight) * batch_size
        # print(weight)
        self.domain_discriminator_accuracy = binary_accuracy(d, torch.where(d_label>0.5, 1, 0))
        
        weight = weight.view_as(d)
