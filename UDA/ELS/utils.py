...
def print_dataset_image_names(concatenated_dataset):
    for dataset in concatenated_dataset.datasets:
        if hasattr(dataset, 'imgs'):
            ...
            print("Unknown dataset structure, please check the dataset implementation.")
            return
...
# change get_dataset function to get a ratio dataset
def get_ratio_dataset(dataset_name, root, source, target, train_source_transform, val_transform, ratio,
    ...
    if dataset_name == "Digits":
        ...
        # load datasets from common.vision.datasets
        dataset = datasets.__dict__[dataset_name]
        dataset_ratio = datasets.__dict__[dataset_name + 'ratio']

        assert len(source) == 2, "Must have two source."

        def concat_dataset(tasks, **kwargs):
            return ConcatDataset([dataset(task=task, **kwargs) for task in tasks])

        # change the second dataset len to the given rate
        first_source_dataset = dataset(root=root, task=source[0], download=True, transform=train_source_transform)
        baseDatasetLength = len(first_source_dataset)
        second_source_dataset = dataset_ratio(root=root, task=source[1], download=True,
                                              transform=train_source_transform, baseDatasetLength=baseDatasetLength,
                                              ratio=ratio)
        train_source_dataset = ConcatDataset([first_source_dataset, second_source_dataset])
        train_target_dataset = concat_dataset(root=root, tasks=target, download=True, transform=train_target_transform)
        val_dataset = concat_dataset(root=root, tasks=target, download=True, transform=val_transform)
        if dataset_name == 'DomainNet':
            test_dataset = concat_dataset(root=root, tasks=target, split='test', download=True, transform=val_transform)
...
def validate_weight(val_loader, model, args, device) -> float:
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    progress = ProgressMeter(
